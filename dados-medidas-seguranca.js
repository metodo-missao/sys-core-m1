...
async function carregarQuestoes() {
    const { data, error } = await supabaseClient
        .from('das_medidas_de_seguranca') // Nome da Tabela
        .select('*')
    ...
    const questoesFormatadas = data.map(q => ({
        ...
        titulo: `Questão ${q.numero_da_questao.toString().padStart(2, '0')} (Inédita/Carreiras Policiais - 2026) - Direito Penal - Das Medidas de Segurança`,
