// dados-legislacao-maria-da-penha-assistencia.js
async function carregarQuestoes() {
    const { data, error } = await supabaseClient
        .from('legislacao_maria_penha_assistencia') // Nome da Tabela no Supabase
        .select('*')
    ...
    const questoesFormatadas = data.map(q => ({
        titulo: `Questão ${q.numero_da_questao.toString().padStart(2, '0')} (Inédita/Carreiras Policiais - 2026) - Lei Maria da Penha (Art. 8º ao 12-C)`,
