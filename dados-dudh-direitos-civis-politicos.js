// dados-dudh-direitos-civis-politicos.js
async function carregarQuestoes() {
    const { data, error } = await supabaseClient
        .from('dudh_direitos_civis_politicos') // Nova tabela no Supabase
        .select('*')
    ...
    const questoesFormatadas = data.map(q => ({
        titulo: `Questão ${q.numero_da_questao.toString().padStart(2, '0')} (Inédita/Carreiras Policiais - 2026) - DUDH (Arts. 3º ao 21)`,
